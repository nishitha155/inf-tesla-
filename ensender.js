...
const keyChainLength = 5;  // Number of intervals
...
const createServer = (port) => {
  const server = net.createServer((socket) => {
    const commitmentKey = generateRandomKey();
    let commitmentKeyHash = hash(commitmentKey);
    let keyChains = [generateKeyChain(keyChainLength).keys];
    socket.on('data', (data) => {
      ...
      if (message.startsWith('NONCE')) {
        ...
        const macData = `${nonce}${currentTime}${commitmentKey1}${commitmentKey2}${startingTime}${timeIntervalDuration}${disclosureDelay}${commitmentKey}`;
    ...
    const intervalId = setInterval(() => {
      ...
      const start = performance.now();
      const chainIndex = Math.floor(currentInterval / keyChainLength);
      const keyIndex=currentInterval%keyChainLength
      ...
      let encryptedNewChainKey=null;
      if(keyIndex==keyChainLength-1&&chainIndex<Math.ceil(totalIntervals/keyChainLength)-1){
        const {keys:newKeyChain,hashChainTime}=generateKeyChain(keyChainLength);
        const newChainFirstKey=newKeyChain[0];
        newcommit=commitmentKey
        encryptedNewChainKey = encryptAES(newChainFirstKey, newcommit);
        newcommit=hash(commitmentKey)
      ...
      if (currentInterval >= disclosureDelay) {
        ...
        const disclosureKeyIndex = disclosureInterval % keyChainLength;
        const disclosedKey = keyChains[disclosureChainIndex][disclosureKeyIndex];
