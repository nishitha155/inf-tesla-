...
const intervalDuration = 12000;  // Interval duration in milliseconds (12 seconds)
const serverPort = 12345;  // Port on which the sender will send data
const lambda = 6;  // Lambda for Poisson distribution (average number of messages per interval)
...
const createSenderServer = (port) => {
  ...
  const server = net.createServer((socket) => {
    ...
    const intervalId = setInterval(() => {
      ...
      const keyDerivationTime = endKeyDerivation - startKeyDerivation;
      totalKeyDerivationTime += keyDerivationTime;
